...
class SegementTree2D(object):
    ...
    def update(self, r, c, node, val):
        if r > node.leftRow or c > node.leftColum:
            return

        if node.rightRow == node.leftRow and node.rightColum == node.rightColum:
            node.areaSum = val
            return
...
print(testClass.sumRegion(1, 1, 2, 2))
print(testClass.sumRegion(1, 2, 2, 4))
